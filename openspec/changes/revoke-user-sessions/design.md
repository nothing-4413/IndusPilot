...
Existing Redis session values keep their current serialization. The new user index is populated on future logins. The first revocation after deployment covers all indexed sessions; old unindexed sessions retain their normal TTL, so deployments should use a session TTL compatible with their credential-rotation policy.
