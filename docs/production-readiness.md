...
- 客户端：Qt 客户端已接入 HTTP 登录、资产列表、运行监控、告警列表、维护工单列表和基础流转；AI 页面仍为离线兜底，工单新建/分派和监控/告警写操作尚未接入。
...
3. Qt 客户端联机化深化：在现有 HTTP 登录、资产、运行监控、告警列表、维护工单列表和基础流转基础上，继续接入 AI 诊断入口、工单新建/分派和监控/告警写操作。
