...
- 客户端：Qt 客户端已接入 HTTP 登录、资产列表与状态更新、运行监控列表与状态写入、告警创建/列表与处置、维护工单列表、新建/从告警生成/分派/基础流转、AI 结构化诊断入口和 AI 交互审计查询与 CSV 导出；工单编辑/附件、告警规则/通知联动和 AI 审计分页尚未接入。
...
3. Qt 客户端联机化深化：在现有 HTTP 登录、资产、运行监控列表与状态写入、告警创建/列表与处置、维护工单列表、新建/从告警生成/分派/基础流转、AI 诊断入口和 AI 交互审计查询与 CSV 导出基础上，继续接入工单编辑/附件、告警规则/通知联动和 AI 审计分页。
