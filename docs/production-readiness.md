...
- 客户端：Qt 客户端已接入 HTTP 登录、资产列表、运行监控、告警列表、维护工单列表、基础流转和 AI 结构化诊断入口；工单新建/分派、监控/告警写操作和 AI 交互历史页面尚未接入。
...
3. Qt 客户端联机化深化：在现有 HTTP 登录、资产、运行监控、告警列表、维护工单列表、基础流转和 AI 诊断入口基础上，继续接入工单新建/分派、监控/告警写操作和 AI 交互历史页面。
