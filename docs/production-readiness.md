...
- 客户端：Qt 客户端已接入 HTTP 登录、资产列表、运行监控、告警列表与处置、维护工单列表、新建/分派/基础流转、AI 结构化诊断入口和 AI 交互审计查询；工单一键从告警生成/编辑/附件、监控写操作、告警创建/规则/通知联动和 AI 审计分页/导出尚未接入。
...
3. Qt 客户端联机化深化：在现有 HTTP 登录、资产、运行监控、告警列表与处置、维护工单列表、新建/分派/基础流转、AI 诊断入口和 AI 交互审计查询基础上，继续接入工单一键从告警生成/编辑/附件、监控写操作、告警创建/规则/通知联动和 AI 审计分页/导出。
