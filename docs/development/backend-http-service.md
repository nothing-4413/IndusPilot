...
- 启用 webhook 时必须配置 `notifications.webhook_allowed_hosts`（逗号分隔的精确主机名）；未列入 allowlist 的目标不会发起网络请求。对应环境变量为 `INDUSPILOT_NOTIFICATIONS_WEBHOOK_ALLOWED_HOSTS`。发送前还会解析地址并拒绝非公网地址，连接使用已校验的 IP，Host 头保留原始主机名，3xx 不会被视为成功；IPv6 目标使用方括号。HTTPS 继续启用证书校验，当前 Drogon IP 客户端不提供独立域名 SNI，依赖域名 SNI 的目标需要后续专用 connector。
