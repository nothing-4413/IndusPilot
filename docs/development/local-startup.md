...
默认构建使用内存仓储和内存会话存储，适合快速验证 Qt 客户端和后端业务模块。客户端会读取 `config/client.example.json` 的 `apiBaseUrl`，后端可用时接入 HTTP 登录和资产列表，后端不可用时保留离线演示数据：
...
生产部署前仍需要替换开发口令和演示盐值、补齐登录失败锁定、真实依赖集成测试、监控指标、外部 AI 推理传输，并继续把运行监控、告警、工单和 AI 页面接入真实 HTTP 数据。
