...
MySQL 初始化脚本会登记以下版本到 `schema_migrations`：`001_foundation_schema`、`002_seed_identity`、`003_runtime_persistence_schema`、`004_work_order_attachments_schema`、`005_alert_rules_notifications_schema`、`006_alert_notification_delivery_schema`。该版本表用于部署核对，不代表后端会在启动时自动迁移数据库。
## HTTP 冒烟测试
...
- MySQL 仓储已经覆盖 identity、asset、alert、work-order、runtime-state 和 AI interaction；生产部署前需执行 `database/mysql/001_foundation_schema.sql`、`002_seed_identity.sql`、`003_runtime_persistence_schema.sql`、`004_work_order_attachments_schema.sql`、`005_alert_rules_notifications_schema.sql`、`006_alert_notification_delivery_schema.sql`，并确认 `schema_migrations` 已登记对应版本。
